...
const ViewShort = () => {
    ...
    useEffect(() => {
        const fetchShort = async () => {
            try {
                ...
                if (!data.success) {
                    throw new Error(data.message || "Failed to fetch short");
                }

                const fetchedShort = data.short;

                setShort(fetchedShort);
                setLiked(Boolean(fetchedShort.isLiked));
                setLikeCount(Number(fetchedShort.likeCount) || 0);
                setCommentCount(Number(fetchedShort.commentCount) || 0);
            } catch (error) {
                console.error(error);
                setError(error?.response?.data?.message || error.message || "Failed to fetch short");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleLike = async () => {
        ...
        try {
            ...
            const response = await toggleLike("short", short._id);
    ...
    const fetchComments = async () => {
        try {
            ...
            const response = await getComments("short", short._id);
    ...
    const handleAddComment = async (event) => {
        ...
        try {
            ...
            const response = await addComment("short", short._id, trimmedComment);
        ...
        return (
            <div className="flex min-h-screen items-center justify-center bg-black text-white">
                <div className="text-center">
                    ...
                    <p className="mt-4 text-sm text-white/60">Loading Short...</p>
                </div>
            </div>
        );
    }
    ...
    if (error || !short) {
        return (
            <div className="flex min-h-screen items-center justify-center bg-black px-5 text-white">
                <div className="text-center">
                    <h2 className="text-lg font-semibold">Unable to load Short</h2>
                    <p className="mt-2 text-sm text-white/50">{error || "Short not found"}</p>
                ...
                <div className="relative h-screen w-full max-w-[500px] overflow-hidden bg-[#111] sm:h-[95vh] sm:rounded-2xl">
                    ...
                    <div className="absolute bottom-6 left-4 right-20 text-white">
                        <div className="flex items-center gap-3">
                            <Avatar
                                username={short?.userId?.username}
                                image={short?.userId?.image}
                            />

                            <div>
                                <p className="text-sm font-semibold">{short?.userId?.username}</p>
                        ...
                        {short.caption && (
                            <p className="mt-3 text-sm leading-5 text-white">{short.caption}</p>
                        )}
                    </div>
